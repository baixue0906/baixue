...
class Run_Main():
    ...
    # 初始化对象，__init__()方法是一种特殊的方法，被称为类的构造函数或初始化方法，当创建了这个类的实例时就会调用该方法
    def __init__(self, token):
        self.TOKEN = token #对象名.属性名=新名字
...
if __name__ == '__main__':
    ...
    data = {
        'guid': 'f7f99efd0bb74c39a13485b7ca09cf07',
        'display_name': 'edit0807'
    }
    token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJleHAiOjE1OTY4NTU0ODYsImlhdCI6MTU5Njc2OTA4NiwibmJmIjoxNTk2NzY4OTY2LCJzeXN0eXBlIjoiIiwidXNlcl9pZCI6IiJ9.hMphegQ0G7S1bs_CeMtr9COM1XfT3llhjyOwUcqutHI'
    run = Run_Main(token)
    ...
    # 将返回结果格式化成字典
    b = json.loads(res)
    # print(b['data']['techGrade'])

    # 调用数据库方法，执行查询语句，输出查询结果
    op_mysql = OperationMysql()
    sql = "SELECT * from W_TeacherInfo WHERE guid='f7f99efd0bb74c39a13485b7ca09cf07'"
    a = op_mysql.search_one(sql)
    # print(a[0]['tech_grade'])

    if b['data']['displayName'] == a[0]['teachername']:
        print('编辑老师接口数据库数据对比成功')
    else:
        print("失败")
    ...
    # 调用数据库方法，执行查询语句，输出查询结果
    op_mysql = OperationMysql()
    sql = "SELECT * from W_TeacherInfo WHERE teachername='王大绩'"
    a = op_mysql.search_one(sql)
    # print(a[0]['tech_grade'])

    if b['data'][1]['certificate_url'] == a[0]['certificate_url']:
        print('查询资质信息接口数据库数据对比成功')
    else:
        print("失败")
