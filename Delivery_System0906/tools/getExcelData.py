...
def get_excelData(sheetName,startRow,endRow):
    resList = []
    #1-excel表路径
    excelDir = '../data/外卖系统接口测试用例-V1.4-2020.09.28.xls'
...
def get_excelData2(sheetName,caseName):
    ...
    for one in workSheet.col_values(0):
        if caseName in one:
            ...
            resList.append((json.loads(reqBodyData),json.loads(respData)))  #封装一个列表里嵌套元组,将excel表里读取的str类型转换成字典
        idx += 1#
    return resList
...
if __name__ == '__main__':
    ...
    for one in get_excelData2('我的商铺','listshopping'):
        print(one)
