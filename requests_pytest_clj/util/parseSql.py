    ...
    def __init__(self, sql_file):
        try:
            self.__conn = pymysql.connect(
                ...
                port=sql_file['port'],
                user=sql_file['user'],
                passwd=sql_file['passwd'],
                db=sql_file['db'],
                charset=sql_file['charset'],
...
if __name__ == '__main__':
    sql_file = config_file().sql_conf()
    ad = Database(sql_file)
    one = ad.select_one('W_UserBaseInfo','username="word001"')
    print(one)
    sql = "select username from W_UserBaseInfo where id = '33976370'"
    two = ad.select_one_ok(sql)
    print(two)
    print(type(two))
