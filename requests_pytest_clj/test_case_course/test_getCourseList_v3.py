...
class TestGetCourseList2():
    ...
    run_data = run_excel.get_all_values_of_sheet(sheet_name01)[12]
    sheet_name03 = run_excel.get_sheet_by_name('getCourseList_v3')
    course_list3 = run_excel.get_all_values_of_sheet(sheet_name03)
    list_run_data = []
    list_run_data.append(run_data)


    @pytest.mark.parametrize('grade,subject,courseType,location,excepted',course_list3)
    @pytest.mark.parametrize('method,url,header',list_run_data)
    def test_getCourseList(self,method,url,header,grade,subject,courseType,location,excepted):
        res = self.course_data.getCourseList3(method,url,header,grade,subject,courseType,location,excepted)
