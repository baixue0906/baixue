...
class TestSelectSql():
    ...
    def test_paid(self):
        ...
        res = self.select_all_ok('select * from W_SalesCourseInfo a, (select courseguid from W_R_Auth_PaidCourse'
                                     ' where userid = 35252159 and activemodule = 1 and isdeleted = 0 and structtype = 3 '
                                 'and isfuture = 0) b where a.guid =b.courseguid and  a.series =1 and '
                                     ' a.modelselectinfo !=" " and a.isvalid =1 and a.name not like "%中考%" ')
        ...
        try:
            for i in res:
                if i['modelselectinfo'] != '':
                    ...
                    #print(moduleinfo)
                    #激活版本为第一个版本
                    moduleinfo = json.loads(moduleinfo)
                    for i in moduleinfo:
                        if '语文' in i:
                            print(i)
...
if __name__ =='__main__':
    pytest.main('-v',['../test_case_course/test_activeCourse.py'])
